...
class JSONSerializer():
    ...
    def serialize(self, obj, **options):
        ...
        self.relatedModFields = options.pop("related", None)
    ...
    def handle_model(self, mod):
        ...
        for related in self.relatedModFields:
            try:
                related_field = getattr(mod,related)
                related_field_qs = related_field.all()
                self.stream.write(u'"links" : ')
                self.handle_queryset(related_field_qs)
                self.stream.write(u'],')
            except:
                pass
...
#        try:
#            #Comment being example model name
#            comment = Comment.objects.get(pk=1)
#        except Comment.DoesNotExist:
#            response.update({'response_code': 2, 'response_string': 'Does not exists',})
#            return json_response_from(response)
#        response.update({'response_code': 1, 'comment': comment,})
#        return json_response_from(response)